#!/usr/bin/env python3
"""
🧠 NEUROCANVAS × TBP.MONTY: HONEST CLIP-GATED HAWKINS HETERARCHY (v280.0)
- Полный стек Хокинса: 16 384 макроколонки (4 узла × 64x64 L4 Sheet на CUDA).
- Интеграция с tbp.monty: каждый шаг формирует CMP Message (location, pose_vectors, confidence).
- Честный шлюз обучения (Hard Gate): imprinting SDR только при уверенности CLIP >= 65%.
- Защита от катастрофического забывания: Frozen Retention Benchmark (все >= 85%).
- Замкнутый цикл Active Inference: SDR -> Monty LM -> SD-LCM -> CLIP Teacher -> Brain PE.
- Поддержка флагов: --sim, --concepts 4/8, --online-learn, --online-lr.
"""
...
# Добавляем пути к модулям проекта и tbp.monty
...
# Импорт официального Message из tbp.monty
try:
...
# ==============================================================================
# 2. ПОТОКОВАЯ ГЕТЕРАРХИЯ ХОКИНСА (16 384 КОЛОНКИ НА CUDA)
# ==============================================================================
...
# ==============================================================================
# 3. ЗРИТЕЛЬНЫЙ ЦЕНЗОР CLIP (ЗАКРЫВАЕТ ПЕТЛЮ ACTIVE INFERENCE)
# ==============================================================================
...
# ==============================================================================
# 4. ВОРКЕР ДИФФУЗИИ SD-LCM (С ПРЯМОЙ ПЕРЕДАЧЕЙ PROMPT EMBEDDINGS)
# ==============================================================================
        ...
        while self.running:
            if not self.initialized:
                try:
                    ...
                    print("✅ [SD-LCM] Прямое соединение с brain_server установлено!")
                except Exception:
                    time.sleep(0.5)
                    continue
            ...
            try:
                ...
                # Передаем prompt_embeds в полном соответствии с протоколом brain_server.py
...
# ==============================================================================
# 5. ГЛАВНЫЙ ИСПОЛНЯЕМЫЙ КОНТУР
# ==============================================================================
def main():
    parser = argparse.ArgumentParser(description="NeuroCanvas × tbp.monty: Honest CLIP-Gated Heterarchy")
    parser.add_argument('--sim', action='store_true', help="Включить автономного агента")
    parser.add_argument('--concepts', type=int, default=4, choices=[4, 8], help="Количество концептов (4 или 8)")
    ...
    if args.sim:
        ...
        print(f"🤖 [CONFIG] Запущен синтетический агент ({NUM_CONCEPTS} концептов)...")

    pygame.init()
    screen = pygame.display.set_mode((WIDTH, HEIGHT))
    pygame.display.set_caption(f"NeuroCanvas × tbp.monty: Honest CLIP-Gated Heterarchy ({NUM_CONCEPTS} Concepts)")
    ...
    # Переменные жесткого шлюза калибровки
    is_calibrating = True
    learn_idx = 0
    clean_steps_accumulated = 0
    CLEAN_STEPS_REQUIRED = 15       # 15 кадров подтверждения
    CLIP_HONEST_THRESHOLD = 0.65    # Порог зрения

    concept_snapshots = {}
    concept_scores = [0.0] * NUM_CONCEPTS
    curriculum_epoch = 1

    # Состояние интеграции пути по Монти (L6)
    monty_location = np.array([0.0, 0.0, 0.0], dtype=np.float64)

    print("🧠 [SYSTEM] Конвейер запущен. Калибровка активна до достижения >= 85% по всем концептам...")
    ...
    try:
        while True:
            ...
            # Считываем реальный RGB-кадр диффузии
            with worker.lock:
            ...
            # Вычисление 16 384-битного SDR Хокинса на GPU
            with torch.no_grad():
                full_sdr, sdr_f3 = heterarchy.get_current_sdr(node_tensors)

            # Формирование сообщения Cortical Messaging Protocol (CMP)
            ...
            # ==================================================================
            # ШЛЮЗ ОБУЧЕНИЯ (CLIP-GATED HARD GATE)
            # ==================================================================
            if is_calibrating:
                ...
                # Впечатываем в память ТОЛЬКО когда зрение подтвердило образ (>= 65%)
                ...
                # Накоплено 15 чистых кадров -> финализация бинарного прототипа
                if clean_steps_accumulated >= CLEAN_STEPS_REQUIRED:
                    ...
                    # Проверка жесткого порога: ВСЕ ДОЛЖНЫ БЫТЬ >= 85%
                    if learn_idx >= NUM_CONCEPTS:
                        ...
                        if min_score >= 85.0 and all(heterarchy.concept_trained > 0):
                            ...
                            print("🏆 [HARD GATE PASSED] Все концепты подтверждены выше 85%! Переход в сёрфинг.")
                        else:
                            curriculum_epoch += 1
                            learn_idx = int(np.argmin(concept_scores))
                            heterarchy.reset_accumulator(learn_idx)
                            print(f"⚠️ [RE-LEARNING] Доучиваем слабейший концепт: [{TARGET_NAMES[learn_idx]}] ({concept_scores[learn_idx]:.1f}% < 85%)")
                ...
                # ==============================================================
                # СВОБОДНЫЙ СЁРФИНГ (ACTIVE INFERENCE ПО ХОКИНСУ)
                # ==============================================================
                with torch.no_grad():
                ...
                # Denoising сила управляется стабильностью Беты и фрустрацией
            ...
            # Центральный холст: реальная фотографическая диффузия
            ...
            # Карта L4 F3 (64x64 бинарных колонок)
            ...
            # 1. ПАНЕЛЬ ОБУЧЕНИЯ / БЕНЧМАРК ХОКИНСА
            ...
            if is_calibrating:
                ...
                for i, name in enumerate(TARGET_NAMES):
                    ...
                    screen.blit(font_s.render(txt_s, True, col_s), (panel_x + 15, panel_y + 118 + i * 20))

                min_sc = min(concept_scores) if any(heterarchy.concept_trained > 0) else 0.0
                screen.blit(font_b.render(f"HARD GATE: ALL MUST BE >= 85.0%", True, (255, 100, 100) if min_sc < 85 else (0, 255, 200)), (panel_x + 15, panel_y + 242))

            else:
                screen.blit(font_b.render("FROZEN RETENTION BENCHMARK (PASSED)", True, (0, 255, 200)), (panel_x + 12, panel_y + 12))
                for i, name in enumerate(TARGET_NAMES):
                    screen.blit(font_s.render(f"{name:10s}: {concept_scores[i]:4.1f}% [LOCKED IN LTM]", True, (100, 255, 100)), (panel_x + 15, panel_y + 45 + i * 24))

                screen.blit(font_b.render("ALL CONCEPTS QUALIFIED >= 85.0%", True, (0, 255, 200)), (panel_x + 15, panel_y + 215))
                screen.blit(font_s.render("16,384 Columns | Pure Sparse HTM Consensus", True, (150, 180, 200)), (panel_x + 15, panel_y + 238))

            # 2. ПАНЕЛЬ ДЕКОДЕРА, МОНТИ И АГЕНТА
            c_x, c_y = 30, 330
            ...
            screen.blit(font_b.render("LIVE ACTIVE INFERENCE & CMP STREAM", True, (100, 180, 255)), (c_x + 12, c_y + 12))
            ...
            if agent:
                ...
                screen.blit(font_b.render(f"Agent Quest    : [{TARGET_NAMES[t_idx]}]", True, quest_col), (c_x + 15, c_y + 88))
                screen.blit(font_s.render(f"Status         : {'CALIBRATING...' if is_calibrating else ('MATCH CONFIRMED' if t_idx == top_idx else 'SEEKING')}", True, quest_col), (c_x + 15, c_y + 110))

                screen.blit(font_s.render(f"Satisfaction   : {sat*100:4.1f}%", True, (100, 255, 100)), (c_x + 15, c_y + 135))
                ...
                screen.blit(font_s.render(f"Frustration    : {fru*100:4.1f}% | Boredom: {bor*100:4.1f}%", True, (255, 80, 80)), (c_x + 15, c_y + 168))
                ...
                screen.blit(font_s.render(f"Theta dPhi/dt  : {frame.theta_freq:.2f} Hz | Delta: {frame.delta_freq:.2f} Hz", True, (200, 220, 240)), (c_x + 15, c_y + 212))
                screen.blit(font_s.render(f"Denoise Power  : {worker.strength:.2f} | SD-LCM FPS: {worker.fps:.1f}", True, (160, 180, 200)), (c_x + 15, c_y + 235))

            # 3. НИЖНЯЯ ПАНЕЛЬ: СПЕКТР 120 РЁБЕР ЗНАКОВОГО iPLV
            BY, BH = 620, 300
